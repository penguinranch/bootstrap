...
6. **Keep `BEST_PRACTICES.md` in Sync:** `BEST_PRACTICES.md` is the entry point for agents applying these standards to existing projects without running the installer. When you add, rename, or remove files in `/templates/`, update its file references and tier tables to match.
